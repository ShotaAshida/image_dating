...
sys.path.append('..')
from dew_dataset import DewCoralObjectDataset
...
def main(rank, world_size):
    ...
    train_dataset = DewCoralObjectDataset(csv_path=train_csv, img_dir=config['image_dir'], transform=train_transform, reverse=True)
    valid_dataset = DewCoralObjectDataset(csv_path=valid_csv, img_dir=config['image_dir'], transform=eval_transform, reverse=True)
    ...
    if rank == 0:
        writer = SummaryWriter(log_dir='../runs/{obj}-ReverseCoral-{now}'.format(obj=obj, lr=config['learning_rate']['coral'], now=now.strftime("%Y%m%d-%H%M%S")))
        print(ddp_model)
