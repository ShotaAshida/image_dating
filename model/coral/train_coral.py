...
#    fc_param = []
#    other_params = []
#    'module.linear_1_bias'
#    for name, param in ddp_model.named_parameters():
#        print(name)
#        if name == 'module.fc.weight' or name == 'module.linear_1_bias':
#            print(name)
...
#    optimizer = optim.SGD([{'params': other_params}, {'params': fc_param, 'lr': config['learning_rate']['fc']}], lr=config['learning_rate']['base'], momentum=config['momentum'])
#    scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=100000, gamma=0.5)
    ...
    train_dataset = DewCoralDataset(csv_path=train_csv, img_dir=config['image_dir'], transform=train_transform)
    valid_dataset = DewCoralDataset(csv_path=valid_csv, img_dir=config['image_dir'], transform=eval_transform)
    test_dataset = DewCoralDataset(csv_path=test_csv, img_dir=config['image_dir'], transform=eval_transform)
    ...
    if rank == 0:
        writer = SummaryWriter(log_dir='../runs/Coral-Adam-valid-lr{lr}'.format(lr=config['learning_rate']['coral']))
        print(ddp_model)
